...
```{r, echo=FALSE, fig.cap="RMSE for all methods", out.width = '100%'}
...
```{r, echo=FALSE, fig.cap="Variable Importance Plot", out.width = '100%'}
...
```{r, echo=FALSE, fig.cap="Partial Dependence Plot", out.width = '100%'}
...
After resampling, 8 models share a common set of resampled datasets. The gam model has largest mean value of ROC. The ROC of all 8 model are showed below.
...
We measure performance with error rate. The train error for GAM is 0.266, the test error for GAM is 0.283,which indicates that the model has good performance on training and test.

# 7 Conclusions and discussion

By a random sample of 4000 participants from the dataset in ”recovery.RData”, a prediction model for recovery time and identify important risk factors for long recovery time is trained by compare four candidate models. There are some findings from the model analysis and discuss the insights gained into predicting time to recovery from COVID-19. Model with smooth terms and pieceswise function seems have better performance than sample linear model on this data. Relationships discovered on data exploration may not be the same as those in the model. Smoking status, vaccinated status, whether infected severely and BMI are significantly associated with recovery time of Patient on this data. The impact of race on recovery time still cannot be ignored.
