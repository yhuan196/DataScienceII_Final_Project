...
```{r}
svmr.grid <- expand.grid(C = exp(seq(-2,6,len=20)),
...
sprintf("The train error for Support vector classifier is %.3f", train.error.svmr)
...
sprintf("The test error for Support vector classifier is %.3f", test.error.svmr)
```

The value is not on the boundary, hence the local maximum accuracy is achieved. It takes a long time (aprox 30 min) to run so it may be hard to refine into a smaller grid.
