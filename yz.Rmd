...
```{r}
ctrlbbb <- trainControl(method = "cv")
...
test error
```{r}
gbmA.pred = predict(gbmA.fit, newdata = OJ[-rowTrain,], type = "raw")
mean(gbmA.pred != y_test_class)
```
...
test error
```{r}
mars.pred = predict(mars.fit , newdata = test_class, type = "prob")[,2]
roc(y_test_class, mars.pred)
```
